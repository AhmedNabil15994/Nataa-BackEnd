...
//                DB::raw($withRefundCount),
    ...
    public function drivers($request)
    {
        $query = User::whereHas('orders',function ($q) use ($request){
                $q->whereHas('orderStatus',function ($q2){
                    $q2->successOrderStatus();
                });

                if((isset($request->req['from']) && !empty($request->req['from'])) && (isset($request->req['to']) && !empty($request->req['to'])) ){
                    $q->whereDate('orders.created_at', '>=', $request->req['from'])->whereDate('orders.created_at', '<=', $request->req['to']);
                }
        ...
        $query = $query->when($request->input('search.value'), function ($query) use ($request) {
            $query->whereExists(function ($query) use ($request) {
                $query->select(DB::raw(1))->where('name', 'like', '%' . $request->input('search.value') . '%');
            });
        });

        if(isset($request->req['company_id']) && !empty($request->req['company_id'])){
            $query = $query->where('company_id',$request->req['company_id']);
        }

        if(isset($request->req['driver_id']) && !empty($request->req['driver_id'])){
            $query = $query->where('id',$request->req['driver_id']);
        }
