...
class DriverRepository
{
    ...
    public function getAllDriversCompanies($order = 'id', $sort = 'desc')
    {
        $users = $this->user->whereHas('roles', function ($query) {
            $query->where('name', 'delivery_company');
        })->orderBy($order, $sort)->get();
        return $users;
    }
    ...
    {
        $user = $this->user->withDeleted() /* ->with('vendors') */->find($id);
        return $user;
    }
    ...
    {
        $query = $this->user/* ->with('vendors') */->where('id', '!=', auth()->id())->whereHas('roles.perms', function ($query) {
            $query->where('name', 'driver_access');
            ...
            /* $query->orWhereHas('vendors.translations', function ($q) use ($request) {
